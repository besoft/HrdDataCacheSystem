...
/*virtual*/ void vtkParametricFunctionSource::ExecuteData(vtkDataObject *output)
{
...
//----------------------------------------------------------------------------
void vtkParametricFunctionSource::Produce1DOutput(vtkDataObject *output)
{
	...
	for (i=0; i<numPts; i++)
	{
		...
		this->ParametricFunction->Evaluate(t,x);
		pts->SetPoint(i,x);
		lines->InsertCellPoint(i);
	}

	//vtkInformation *outInfo = output->GetInformationObject(0);
	vtkPolyData *outData = static_cast<vtkPolyData*>(output);
...
//----------------------------------------------------------------------------
void vtkParametricFunctionSource::Produce2DOutput(vtkDataObject *output)
{
	...
	double uStep = ( MaxU - this->ParametricFunction->GetMinimumU() ) / (PtsU - 1);
	double vStep = ( MaxV - this->ParametricFunction->GetMinimumV() ) / (PtsV - 1);
	...
	int k = 0;
	double uv[3];
	uv[2] = 1.0;	//if Onion is enabled, we want to return correct values.
	...
	for ( int i = 0; i < PtsU; ++i )
	{
		...
		for ( int j = 0; j < PtsV; ++j )
		{
			...
			this->ParametricFunction->Evaluate(uv,Pt);

			// Insert the points and normal.
			points->InsertPoint(k, Pt[0], Pt[1], Pt[2]);

			// Calculate the normal.
			if ( this->ParametricFunction->GetDerivativesAvailable())
			{
				this->ParametricFunction->EvaluateDerivative(uv, Du);
	...
	cp->SetConvertStripsToPolys(0);
	cp->SetConvertPolysToLines(1);
	cp->SetConvertLinesToPoints(0);  
	cp->SetToleranceIsAbsolute(1);
	cp->SetAbsoluteTolerance(1e-6);	
	cp->SetInput(tri->GetOutput());	
	cp->Update();

	//vtkInformation *outInfo = output->GetInformationObject(0);
	vtkPolyData *outData = static_cast<vtkPolyData*>(/*outInfo->Get( vtkDataObject::DATA_OBJECT() )*/output);
	outData->DeepCopy(cp->GetOutput());
	
	outData->SetLines(NULL);	//CleanPolyData could create lines, we do not want them!
