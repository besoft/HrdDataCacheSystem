...
vtkCxxRevisionMacro(vtkMAFPolyDataCutOutFilterEx, "$Revision: 1.1.2.2 $");
...
/*virtual*/ unsigned long int vtkMAFPolyDataCutOutFilterEx::GetMTime()
	//------------------------------------------------------------------------
{
...
/*virtual*/ void vtkMAFPolyDataCutOutFilterEx::InitMesh()
	//------------------------------------------------------------------------
{
	...
	//------------------------------------------------------------------------
{
	...
	else
	{
		...
		_ASSERT(nValidTriangles != 0 && nValidTriangles != nTriangles);
...
#pragma region VERTEX SPLIT
			...
			else
			{
				...
				for (int iCurTri = 0; iCurTri < nValidTri; iCurTri++)
				{
					...
					double* A = m_Vertices[pIdsPtr[i]].dCoord;	//RELEASE NOTE: cannot move before the for cycle as we modify m_Vertices here!
...
{	
	...
	{    
		...
		while (tol <= 1e-8 &&
			(nTriId = GetInput()->FindCell(x, NULL, NULL, -1, tol, subId, x_par, weights)) < 0)
		{
